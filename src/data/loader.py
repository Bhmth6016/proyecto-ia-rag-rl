...
def _load_all_files(limit: int | None = None) -> List[Dict[str, Any]]:
    all_products = []
    ...
    for file_idx, file_path in enumerate(jsonl_files):
        ...
        try:
            logger.info(f"Procesando ({file_idx+1}/{len(jsonl_files)}): {file_path.name}")
            ...
            logger.info(f"Total acumulado: {len(all_products)}")
