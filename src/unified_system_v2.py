...
try:
    ...
    BaseSystem = UnifiedRAGRLSystem
except ImportError:
    logger.warning("No se pudo importar UnifiedRAGRLSystem, usando clase base")
    ...
    class BaseSystem:
        ...
        def initialize_from_raw_all_files(self, limit: int = 10000) -> bool:
            return False
...
try:
    ...
    logger.error("No se pudieron importar módulos NER")
    NER_available = False

class UnifiedSystemV2(BaseSystem):
    ...
    def initialize_with_ner(self, limit: Optional[int] = None, 
                          use_cache: bool = True,
        ...
        if not self.canonical_products:
            ...
            if base_cache.exists():
                ...
                if hasattr(self, 'initialize_from_raw_all_files'):
                    limit_val = limit if limit is not None else 10000
                    if not self.initialize_from_raw_all_files(limit=limit_val):
                        logger.error("Error inicializando sistema base")
                        return False
                else:
                    logger.error("Método initialize_from_raw_all_files no disponible")
                    return False
    ...
    def _method_rlhf(self, query_text: str, k: int) -> List:
        ...
        if not self.rl_ranker or (hasattr(self.rl_ranker, 'has_learned') and not self.rl_ranker.has_learned):
    ...
    def _method_full_hybrid(self, query_text: str, k: int) -> List:
        ...
        if not has_ner or not has_rlhf:
    ...
    def train_rlhf_with_queries(self, train_queries: List[str], 
        ...
        if trained > 0:
            if hasattr(self.rl_ranker, 'get_stats'):
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        if hasattr(super(), 'get_system_stats'):
            stats = super().get_system_stats()
        else:
            stats = {}
        
        ner_enriched_count = 0
        for product in self.canonical_products:
            if hasattr(product, 'ner_attributes') and product.ner_attributes:
                ner_enriched_count += 1
        
        stats.update({
            'has_ner_ranker': self.ner_ranker is not None,
            'has_ner_extractor': self.ner_extractor is not None,
            'ner_enriched_count': ner_enriched_count,
