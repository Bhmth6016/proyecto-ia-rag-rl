...
class EnhancedJSONEncoder(json.JSONEncoder):
    def default(self, o: Any) -> Any:
        # Usamos los tipos base de numpy en lugar de los específicos
...
def save_results(results: Dict[str, Any], summary: Dict[str, Any], 
    ...
    for method, stats in summary.items():
        ...
        for key, value in stats.items():
            if isinstance(value, np.floating):
                ...
                save_data['summary'][method][key] = bool(value)
            elif isinstance(value, np.ndarray):
                save_data['summary'][method][key] = value.tolist()
            else:
    ...
    for method, test in tests.items():
        ...
        for key, value in test.items():
            if isinstance(value, np.floating):
                ...
                save_data['statistical_tests'][method][key] = value.tolist()
            else:
    ...
    with open(txt_file, 'w', encoding='utf-8') as f:
        ...
        for method in ['ner_enhanced', 'rlhf', 'full_hybrid']:
            if method in tests:
                sig = "SIGNIFICATIVO" if tests[method].get('significant', False) else "NO SIGNIFICATIVO"
